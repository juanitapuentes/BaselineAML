...
def main():
    ...
    if args.use_prot:
        model = PLANet(args).to(device) #en teoría, se podría borrar este if
    else:
    ...
    # Load pre-trained LM module
    #elif args.LMPM: modificaciones Nov03
    elif args.solo_LM: #aca no entra
        #breakpoint() #borrar
        best_molecule_path = args.model_load_init_path
        model_path = (
            "/media/SSD5/pruiz/2022-2023/LM_102/Pretrained-Model/Fold{}/model_ckpt/BS_2560-NF_full_valid_best.pth".format(args.cross_val)
        )
        ...
        for k, v in pre_model["model_state_dict"].items():
            if args.use_prot: #se puede borrar, no lo vamos a usar
